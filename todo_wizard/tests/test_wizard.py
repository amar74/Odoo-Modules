...
from odoo.tests.common import TransactionsCase
...
class TestWizard(TransactionCase):
	...
	def test_papulate_tasks(self):
		"Populate tasks buttons should add two task"
		self.wizard.do_populate_tasks()
		count = len(self.wizard.task_ids)
		self.assertEqual(count, 2, 'Wrong number of populated tasks')
		self.assertItemsEqual(self.wizard.task_ids, expected_tasks, 'Incorrect set of populated tasks')
